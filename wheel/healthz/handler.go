...
func toggleStartupStatus(w http.ResponseWriter, _ *http.Request) {
	...
	_, _ = w.Write([]byte("Enable /healthz/startup -> " + strconv.FormatBool(startupStatus)))
}
...
func toggleLivenessStatus(w http.ResponseWriter, _ *http.Request) {
	...
	w.WriteHeader(http.StatusOK)
	w.Header().Set("Content-Type", "text/plain")
	_, _ = w.Write([]byte("Enable /healthz/liveness -> " + strconv.FormatBool(startupStatus)))
}
...
func toggleReadinessStatus(w http.ResponseWriter, _ *http.Request) {
	...
	w.WriteHeader(http.StatusOK)
	w.Header().Set("Content-Type", "text/plain")
	_, _ = w.Write([]byte("Enable /healthz/readiness -> " + strconv.FormatBool(startupStatus)))
}
